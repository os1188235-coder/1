...
import { Card, Enemy, PlayerStats, LeaderboardEntry, GamePhase } from '../types';
...
export default function GameScreen({ initialShowLeaderboard = false }: { initialShowLeaderboard?: boolean }) {
  // Game Setup
  ...
  const [currentStage, setCurrentStage] = useState<number>(1); // Total 1 to 30 Stages (10 per Chapter)
  ...
  useEffect(() => {
    setShowLeaderboardTab(initialShowLeaderboard);
    if (initialShowLeaderboard) {
      setPhase('menu');
    }
  ...
  const getChapterOfStage = (stageNum: number): ChapterInfo => {
    if (stageNum <= 10) return CHAPTERS[1];
    if (stageNum <= 20) return CHAPTERS[2];
    return CHAPTERS[3];
  };
  ...
  const handlePlayCard = (card: Card) => {
    if (stats.energy < card.cost) {
      triggerFloatingText("마나 결핍!", "damage", "player");
      return;
    }
    if (!enemy) return;
    ...
    const nextEnergy = stats.energy - card.cost;
    ...
    if (card.damage !== undefined && card.damage > 0) {
      const strikeHits = card.templateId === 'swift_slash' ? 2 : 1;
      let totalInflicted = 0;

      for (let h = 0; h < strikeHits; h++) {
        let finalDam = card.damage + stats.strength;
    ...
    // 2. Shield/Block defense resolution
    if (card.block !== undefined && card.block > 0) {
      nextPlayerBlock += card.block;
      setScreenFlash('shield');
      setTimeout(() => setScreenFlash(null), 150);
      triggerFloatingText(`+${card.block} 방벽`, 'shield', 'player');
    }
    ...
    if (card.heal !== undefined && card.heal !== 0) {
      if (card.heal > 0) {
        ...
        // self harm
        nextPlayerHp = Math.max(1, nextPlayerHp + card.heal); // do not let self harm kill immediately
        triggerFloatingText(`자해 ${card.heal}`, 'damage', 'player');
      }
    }

    // 4. Strength augmentation
    if (card.strength !== undefined && card.strength > 0) {
      nextPlayerStrength += card.strength;
      triggerFloatingText(`힘 +${card.strength}`, 'buff', 'player');
    }
    ...
    } else if (card.drawBonus !== undefined && card.drawBonus > 0) {
  ...
  const advanceStagePath = () => {
    if (currentStage === 30) {
      // Victory the entire campaign saga!
      setStats(prev => ({
        ...prev,
        score: prev.score + 3500 // Legendary win bonus
      }));
      setTimerActive(false);
      setPhase('victory');
    } else {
  ...
  const handleBuyShopCard = (card: Card) => {
    ...
    setStats(prev => ({ ...prev, gold: prev.gold - cost, score: prev.score + 80 }));
    setDeck(prev => [...prev, card]);
  ...
  const handleRemoveCardInShop = (cardId: string) => {
    ...
    setStats(prev => ({ ...prev, gold: prev.gold - removeCost, score: prev.score + 50 }));
    setAlchemyTargetCardId(null);
  ...
  const handleApplyRuneToCard = (cardId: string, runeType: 'cataclysm' | 'aegis' | 'ascension' | 'vitality') => {
    ...
    setStats(prev => ({ ...prev, gold: prev.gold - cost, score: prev.score + 100 }));
  ...
  const handleSubmitHighScore = () => {
    ...
    const newHofRecord: LeaderboardEntry = {
      ...
      score: stats.score + Math.floor(stats.gold / 2),
    ...
    <div id="antique_game_controller_viewport" className="w-full flex-1 flex flex-col space-y-6 relative select-none">
        ...
        <div className="flex items-center gap-10">
          ...
          <div className="text-left">
            ...
            <div className="flex items-center gap-2 mt-0.5">
              ...
              <strong className="font-display text-sm text-stone-200 uppercase tracking-wider">
                기장 {currentStage} / 30 관문
              </strong>
            </div>
          </div>
          ...
          <div className="flex-1 vintage-panel-gold p-8 rounded-xl flex flex-col justify-between space-y-6 relative overflow-hidden group">
            ...
            <div className="space-y-4">
              ...
              <p className="text-stone-300 font-serif text-sm leading-relaxed">
                30개의 장엄한 연대기 관문을 격파하고 심연의 황좌를 탈취해야 합니다. 
                이 여정은 약 1시간이 소요되며, 덱의 완벽한 밸런스와 금전 활용, 마법공학 룬 조각 융질 전략이 필요합니다.
              </p>
            </div>

            {/* Quick Hero Draft Selector */}
            <div className="space-y-3 pt-6 border-t border-amber-900/30">
              <label className="block text-xs uppercase tracking-wider text-[#c2965e] font-display font-bold">도전자 서명 기입</label>
              <div className="flex gap-2">
                <input 
                  type="text" 
                  placeholder="모험가의 이름을 기입하십시오..." 
                  value={playerName}
                  onChange={(e) => setPlayerName(e.target.value)}
                  className="flex-1 bg-stone-900 border gold-border px-4 py-2 text-stone-100 rounded text-sm font-serif focus:outline-none focus:ring-1 focus:ring-amber-500"
                />
              </div>
            </div>
            ...
            <div className="pt-4">
              ...
              >
                ...
                <span>심연의 원정 출범 (Embark Journey)</span>
              </button>
            </div>
          </div>
      ...
      {phase === 'map' && (
        <div id="antique_campaign_map" className="space-y-6 w-full flex-1">
          ...
          {/* Deck Overview Screen inside campaign to help calculation */}
          <div className="vintage-panel p-6 rounded-xl space-y-4">
      ...
      {/* 3. THE IMMERSIVE BATTLEFIELD HUD (LOGS REMOVED ENTIRELY) */}
      {phase === 'battle' && enemy && (
        <div id="antique_battle_arena" className="space-y-6 w-full flex-1">
          
          {/* Combatants Dual Overlay Grid */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-stretch">
            
            {/* Player details panel */}
            <div className="vintage-panel p-6 rounded-xl border-l-4 border-l-amber-600 space-y-4 relative overflow-hidden shadow-inner">
              <div className="flex justify-between items-center">
                <span className="text-[10px] font-display text-[#c2965e]/60 tracking-widest uppercase">아군 정예 선열</span>
                <span className="text-xs font-mono bg-stone-900 text-[#d4af37] px-2 py-0.5 rounded border border-stone-800">
                  아군 힘 증강력: +{stats.strength}
                </span>
              </div>

              <div>
                <h3 className="font-display font-black text-[#d4af37] text-lg">{playerName}</h3>
                <p className="text-[10px] text-stone-400 font-serif italic">현재의 상태 장막 장전</p>
              </div>

              {/* Health and block indicators */}
              <div className="space-y-2">
                <div className="flex justify-between text-xs font-serif">
                  <span className="text-stone-400">전술 체력 잔량</span>
                  <strong className="text-[#d4af37]">{stats.hp} / {stats.maxHp} HP</strong>
                </div>
                <div className="w-full h-4 bg-stone-900 border border-amber-900/40 rounded overflow-hidden relative">
                  <div 
                    className="bg-gradient-to-r from-red-800 to-rose-600 h-full rounded-sm transition-all duration-300"
                    style={{ width: `${Math.max(0, (stats.hp / stats.maxHp) * 100)}%` }}
                  />
                  {stats.block > 0 && (
                    <div 
              ...
              {/* Shields block info */}
              <div className="flex items-center gap-3.5 text-xs font-serif">
                {stats.block > 0 ? (
                  <div className="flex items-center gap-1.5 font-bold text-blue-400 bg-blue-950/40 px-2.5 py-1 rounded border border-blue-500/30 animate-pulse">
                    <Shield className="w-4 h-4 text-blue-400 shrink-0" />
                    <span>누적 방벽 흡수 쉴드: {stats.block}</span>
                  </div>
                ) : (
                  <span className="text-stone-500 italic text-[11px]">가용 활성 방벽 배리어 없음</span>
                )}
              </div>
            </div>

            {/* Enemy monster details panel */}
            <div className="vintage-panel p-6 rounded-xl border-r-4 border-r-red-800 space-y-4 relative overflow-hidden shadow-inner">
              <div className="flex justify-between items-center">
                <span className="text-[10px] font-display text-rose-500/60 tracking-widest uppercase">
                  {enemy.difficulty === 'boss' ? 'CRITICAL STAGE BOSS' : 'CITADEL DEFENDER'}
                </span>
                <span className="text-xs font-mono bg-stone-900 text-rose-500 px-2 py-0.5 rounded border border-stone-800">
                  적 공격 무적 무력: +{enemy.strength}
                </span>
              </div>

              <div>
                <h3 className="font-display font-black text-stone-200 text-lg uppercase tracking-wide">{enemy.name}</h3>
                <p className="text-[10px] text-stone-400 font-serif italic">심연 침식 생명력 지형</p>
              </div>

              {/* Health bar enemy */}
              <div className="space-y-2">
                <div className="flex justify-between text-xs font-serif">
                  <span className="text-stone-400">적 괴수 생명력</span>
                  <strong className="text-rose-400">{enemy.hp} / {enemy.maxHp} HP</strong>
                </div>
                <div className="w-full h-4 bg-stone-900 border border-amber-900/40 rounded overflow-hidden relative">
                  <div 
                    className="bg-gradient-to-r from-red-950 to-red-700 h-full rounded-sm transition-all duration-300"
              ...
              {/* EXTREMELY INTUITIVE ENEMY ACTIONS INTENT SYSTEM */}
              <div className="p-3 bg-stone-900/60 rounded-lg border border-stone-800 flex items-center justify-between text-xs font-serif select-none">
                <div className="flex items-center gap-2">
                  <span className="font-display text-[10px] bg-red-950 text-red-400 px-1.5 py-0.5 border border-red-800/40 rounded uppercase font-bold shrink-0">
                    전술 도해도 (Intent)
                  </span>
                  <span className="text-stone-200 line-clamp-1">{enemy.intent.text}</span>
                </div>

                <div className="shrink-0 p-1 bg-red-950 text-red-500 rounded border border-red-800/30 animate-pulse">
          ...
          {/* ACTIVE FLOATING LABELS HUD (visual feedback center) */}
          <div className="relative min-h-[50px] overflow-visible w-full flex items-center justify-center pointer-events-none">
            {floatingTexts.map(f => (
              <span 
                key={f.id}
                className={`absolute text-sm font-display font-black tracking-widest bg-stone-950/80 px-2.5 py-1 rounded border shadow-lg animate-bounce ${
                  f.type === 'damage' ? 'text-red-500 border-red-800/50' :
                  f.type === 'shield' ? 'text-blue-400 border-blue-800/50' :
                  f.type === 'heal' ? 'text-emerald-400 border-emerald-800/50' :
                ...
                return (
                  <button
                    ...
                    className={`vintage-card flex flex-col justify-between w-36 h-48 p-3 rounded-lg hover:-translate-y-5 transition-all duration-300 text-left border relative select-none disabled:opacity-35 disabled:pointer-events-none cursor-pointer ${getCardFrame(card.rarity)} ${
                      isPlaying ? 'scale-95 bg-amber-500/10' : ''
                    }`}
                    style={{
                      zIndex: 20 + index
                    }}
                  >
                    ...
                    {/* Middle title */}
                    <div className="my-2 flex-1 flex flex-col justify-center items-center text-center">
                      <div className="p-1 px-2.5 rounded bg-stone-900/60 text-[10px] text-stone-400 font-mono italic w-full">
                        {card.rarity.toUpperCase()}
                      </div>
                      <h4 className="text-xs font-bold text-stone-200 mt-2 tracking-wide font-display w-full truncate">
                        {card.name}
                      </h4>
                    </div>

                    {/* Footer description */}
                    <div className="mt-auto border-t border-stone-900 pt-1.5">
                      <p className="text-[10px] text-stone-300 font-serif leading-tight line-clamp-3">
                        {card.description}
                      </p>
                    </div>
                  </button>
      ...
      {phase === 'gameover' && (
        <div id="gameover_screen" className="vintage-panel-gold max-w-lg mx-auto p-8 text-center space-y-6 relative overflow-hidden animate-fadeIn border-t-4 border-t-red-650">
          ...
          <div className="p-5 bg-stone-950/70 rounded-lg border gold-border text-left text-xs font-serif space-y-2.5 shadow-inner leading-relaxed">
            ...
            <div className="flex justify-between">
              <span>도달 성채 단계:</span> 
              <strong className="text-[#d4af37]">Stage {currentStage} / 30 관문</strong>
            </div>
            <div className="flex justify-between">
              <span>소지 수거 금색:</span> 
            ...
            <div className="flex justify-between">
              ...
              <strong className="text-indigo-400">{deck.length}장장</strong>
            </div>
            <div className="flex justify-between">
              <span>클락 누수 시간:</span> 
        ...
        <div id="victory_screen" className="vintage-panel-gold max-w-lg mx-auto p-8 text-center space-y-6 relative overflow-hidden animate-fadeIn border-t-4 border-t-[#d4af37]">
          ...
          <div className="space-y-2">
            ...
            <p id="victory_subtitle" className="text-xs text-emerald-400 font-serif italic font-bold">
              성채 삼종 장벽을 완전히 분쇄하고 공허의 왕좌를 제국령으로 복원했습니다!
            </p>
          </div>
          ...
          <div className="p-5 bg-stone-950/70 rounded-lg border gold-border text-left text-xs font-serif space-y-2.5 shadow-inner leading-relaxed">
            ...
            <div className="flex justify-between">
              <span>관문 정벌 상태:</span> 
              <strong className="text-emerald-400">개선 통과 (Stage 30)</strong>
            </div>
            <div className="flex justify-between">
              <span>개선 금색 잔량:</span> 
              <strong className="text-yellow-500">{stats.gold}골드</strong>
